    ...
    with open(file, 'r') as f, open(out_output, 'w') as w:
        for line in tqdm(f):
            ...
            if total_counter < total_sample:
                if len(j_line["tagged_text"])> 50 and  len(j_line["tagged_text"])<3000:#remove the too long and too short paragraphs
                    if "<TIMEX3" in j_line["tagged_text"]:

                        soup = BeautifulSoup(j_line["tagged_text"], "lxml")
                        ...
                        for labels in soup.findAll("timex3"):
                            ...
                            j_line["tagged_text"]= temp

                        j_line["has_date"] = True
                        if date_counter < total_date:
                            date_counter += 1
                            total_counter += 1
                            w.write(str(json.dumps(j_line)) + "\n")
                    else:
                        j_line["has_date"] = False
                        if no_date_counter < total_no_date:
                            no_date_counter += 1
                            total_counter += 1
                            w.write(str(json.dumps(j_line)) + "\n")
                else:
                    if len(j_line["tagged_text"]) < 50:
                        short_examples=short_examples+1
                    if   len(j_line["tagged_text"]) > 3000:
...
if __name__ == "__main__":
    ...
    percentage_no_date = 0.2# we want 20 percent of the data to have no dates (negative samples)
